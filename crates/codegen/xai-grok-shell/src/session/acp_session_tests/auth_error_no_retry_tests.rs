...
fn session_token_auth_gate_truth_table() {
    ...
    for fp in [false, true] {
        ...
        // Genuine per-model Byok never attaches the session bearer.
        assert!(!gate(true, ModelByok::Byok, fp));
    }
    // Session method + NotByok/Unknown: refresh only on first-party hosts.
    // Third-party base_urls must keep their own key (env-injected DeepSeek/
    // OpenRouter can look like NotByok when absent from [model.*]).
